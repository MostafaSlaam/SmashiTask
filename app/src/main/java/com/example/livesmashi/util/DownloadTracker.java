  ...
  public void toggleDownload(
    ...
    if (download != null && download.state != Download.STATE_FAILED) {
      DownloadService.sendRemoveDownload(
          context, VideoDownloadService.class, download.request.id, /* foreground= */ false);
    } else {
  ...
  private final class StartDownloadDialogHelper
    ...
    public void onPrepared(@NonNull DownloadHelper helper) {
      ...
      if (!hasSchemaData(format.drmInitData)) {
        ...
        Log.e(
            TAG,
            "Downloading content where DRM scheme data is not located in the manifest is not supported");
        return;
      }
    ...
    private DownloadRequest buildDownloadRequest() {
      return downloadHelper
          .getDownloadRequest(Util.getUtf8Bytes((String) mediaItem.mediaMetadata.title))
